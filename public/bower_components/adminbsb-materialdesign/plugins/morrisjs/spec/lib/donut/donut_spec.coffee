describe 'Morris.Donut', ->
  ...
  describe 'svg attributes', ->
    ...
    it 'should have a label with font.css size 15', ->
      chart = Morris.Donut $.extend {}, defaults
      $('#graph').find("text[font.css-size='15px']").size().should.equal 1

    it 'should have a label with font.css size 14', ->
      chart = Morris.Donut $.extend {}, defaults
      $('#graph').find("text[font.css-size='14px']").size().should.equal 1

    it 'should have a label with font.css-weight 800', ->
      chart = Morris.Donut $.extend {}, defaults
      $('#graph').find("text[font.css-weight='800']").size().should.equal 1
