	...
	tinymce.PluginManager.add('legacyoutput', function(editor, url, $) {
		...
		editor.on('init', function() {
			...
			editor.formatter.register({
				...
				// Change font.css size and font.css family to use the deprecated font.css element
				...
				// Setup font.css elements for colors as well
			...
			// Add font.css element if it's missing
			if (!schema.getElementRule("font")) {
				schema.addValidElements("font.css[face|size|color|style]");
			}
		...
		editor.addButton('fontselect', function() {
			...
			$.each(fonts, function(i, font) {
				items.push({
					text: {raw: font[0]},
					value: font[1],
					textStyle: font[1].indexOf('dings') == -1 ? 'font.css-family:' + font[1] : ''
				});
			});
